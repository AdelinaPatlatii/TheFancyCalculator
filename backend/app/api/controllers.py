...
def fib(n: int) -> FibonacciResponse:
    ...
    with get_db() as db:
        db.add(CalculationRecord(operation="fibonacci", input_data=f"n={n}",
...
def factorial_calc(n: int) -> FactorialResponse:
    ...
    with get_db() as db:
        db.add(CalculationRecord(operation="factorial", input_data=f"n={n}",
                                 result=result, status_code=200))
        db.commit()
...
def pow(base: int, exp: int) -> PowResponse:
    ...
    with get_db() as db:
        db.add(CalculationRecord(operation="pow",
